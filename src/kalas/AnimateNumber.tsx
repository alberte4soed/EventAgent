/**
 * <AnimateNumber> — digits that slide + blur only when they change, in the
 * spirit of iOS's numericText content transition. Adapted (single file, no
 * deps) for the Kalas / Vite build. See the original for the full rationale.
 */
...
@keyframes an-resolve {
  from { opacity: 0; filter: blur(var(--an-blur, 16px)); }
  to { opacity: 1; filter: blur(0); }
}
@keyframes an-dissolve {
  from { opacity: 1; filter: blur(0); }
  to { opacity: 0; filter: blur(var(--an-blur, 16px)); }
}
...
export function AnimateNumber({
  ...
  return (
    <span {...rest} className={cn('an-root', className)}>
      ...
      {chars.map((ch, i) => (
        <CharSlot key={len - 1 - i} char={ch} direction={direction} durationMs={duration} blur={blur} />
      ))}
