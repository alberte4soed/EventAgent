...
/** Optional tone overrides sent to the AI on top of the template's fixed voice. */
const TONES = [
  { id: 'formal', da: 'Formel', en: 'Formal' },
  { id: 'warm', da: 'Varm', en: 'Warm' },
  { id: 'short', da: 'Kort', en: 'Short' },
  { id: 'playful', da: 'Legende', en: 'Playful' },
] as const;
      ...
      <div className="order-2 px-6 py-8 sm:px-9 lg:order-1 lg:px-12">
        ...
        <div className="mt-6 rounded-2xl rule bg-card p-4">
          ...
          <textarea
            ...
            rows={2}
            placeholder={da ? 'F.eks. havebryllup ved havet, uformelt og hjerteligt…' : 'e.g. a garden wedding by the sea, informal and heartfelt…'}
            className="mt-3 w-full rounded-lg rule bg-canvas px-3 py-2 text-[0.85rem] text-ink focus:outline-none resize-none"
          />
          <div className="mt-3">
            <FieldLabel>{t('Tone (valgfri)')}</FieldLabel>
            <div className="flex flex-wrap gap-2">
              {TONES.map((tn) => (
                <button
                  key={tn.id}
                  type="button"
                  onClick={() => setTone((cur) => (cur === tn.id ? null : tn.id))}
                  className={`rounded-full border px-3 py-1.5 text-[0.72rem] transition-all cursor-pointer ${
                    tone === tn.id ? 'border-ink bg-ink text-canvas' : 'rule text-ink-soft hover:text-ink'
                  }`}
                >
                  {da ? tn.da : tn.en}
                </button>
              ...
              <div className="grid grid-cols-3 gap-3">
                {variants.map((v, i) => (
                  <div key={i} role="button" tabIndex={0}
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); chooseVariant(v); } }}
                    className="group flex flex-col items-center gap-2 rounded-xl rule bg-card p-2.5 transition-colors hover:bg-shell cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-ink/40">
                    <div className="overflow-hidden rounded-lg transition-transform duration-300 group-hover:-translate-y-0.5">
                      <ScaledPhone width={92}>
                        <PhoneFrame>
                          <Component data={v} />
                        </PhoneFrame>
                      </ScaledPhone>
                    </div>
                    <span className="text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-muted">{t('Variant')} {i + 1}</span>
                  </div>
                ))}
              </div>
            </motion.div>
