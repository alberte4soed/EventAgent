...
/* ── Send moment — unlock → review → sent w/ undo (user is already
   signed in via Supabase before the app mounts) ──────────────────────── */
function SendMoment({ open, onClose, onSent }: {
  open: boolean; onClose: () => void; onSent: () => void;
}) {
  const { paid, setPaid } = useKalas();
  const [step, setStep] = useState<'unlock' | 'review' | 'sent'>('unlock');
  ...
  useEffect(() => {
    if (open) setStep(!paid ? 'unlock' : 'review');
  }, [open]);
